    ...
    size_t payload_size)
{
    ...
    {
        logw(ERROR, "BMP file not big enough to store, bits in payload: %zu // bits available: %zu", payload_size * 8, (size_t)metadata->info.header.height * metadata->info.header.width * 3 * LSB_SIZES[mode]) return -1;
    }
    ...
    size_t payload_size)
{
    ...
    {
        logw(ERROR, "BMP file not big enough to store, bits in payload + mask: %zu // bits available: %zu", payload_size * 8 + 4, (size_t)metadata->info.header.height * metadata->info.header.width * 3) return -1;
    }
